...
final class APICaller {
    ...
    public func getCurrenctUserAlbums(completion: @escaping (Result<[Album], Error>) -> Void) {
        createRequest(
        ...
        createRequest(with: URL(string: Constants.baseAPIURL + "/browse/categories?limit=50"),
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                ...
                do {
                    ...
                    //JSONSerialization.jsonObject(with: data, options: .allowFragments)
                    print("getCategories API: \(result.categories.items)")
                    completion(.success(result.categories.items))
                }
                catch {
                    print("getCategories API: \(error.localizedDescription)")
                    completion(.failure(error))
                }
            }
            task.resume()
        ...
        createRequest(with: URL(string: Constants.baseAPIURL + "/browse/categories/\(category.id)/playlists?limit=50"),
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                ...
                do {
                    ...
                    print("getCategoryPlaylists API: \(playlists)")
                    completion(.success(playlists))
                }
                catch {
                    print("getCategoryPlaylists API: \(error.localizedDescription)")
                    completion(.failure(error))
                }
            }
            task.resume()
    ...
    public func search(with query: String, completion: @escaping (Result<[SearchResult], Error>) -> Void) {
        createRequest(
            ...
            type: .GET) { request in
                print("Search API Request URL - \(request.url?.absoluteString ?? "NONE")")
                let task = URLSession.shared.dataTask(with: request) { data, _, error in
                    guard let data = data, error == nil else {
                        completion(.failure(APIError.failedToGetData))
                        return
                    ...
                    do {
                        ...
                        print ("Search API - \(error.localizedDescription)")
                        completion(.failure(error))
                    }
                }
                task.resume()
    ...
    public func getCurrenctUserPlaylists(completion: @escaping (Result<[Playlist], Error>) -> Void) {
        createRequest(
            ...
            type: .GET) { request in
                let task = URLSession.shared.dataTask(with: request) { data, _, error in
                    ...
                    do {
                        ...
                        //JSONSerialization.jsonObject(with: data, options: .allowFragments)
                        print("getCurrencyUserPlaylists API JSON Response - \(result)")
                        completion(.success(result.items))
                    }
                    catch {
                        print("getCurrencyUserPlaylists API onError - \(error)")
                        completion(.failure(error))
                    }
                }
                task.resume()
    ...
    public func createPlaylist(with name: String, completion: @escaping (Bool) -> Void) {
        getCurrentUserProfile { [weak self] result in
                ...
                self?.createRequest(with: URL(string: urlString), type: .POST) { baseRequest in
                    ...
                    let task = URLSession.shared.dataTask(with: request) { data, _, error in
                        ...
                        do {
                            ...
                            print("createRequest API JSON Response - \(result)")
            ...
            case .failure(let error):
                print("getCurrentUserProfile API onError - \(error.localizedDescription)")
            }
        }
    }
